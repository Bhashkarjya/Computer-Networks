...
int main()
{
    ...
    while(1){
        cout<<"Client 2: Enter the message that you want to send to the server\n";
        fgets(send_msg,MAX_DATA,stdin);
        int msg_length = strlen(send_msg) + 1;
        send(client_fd,send_msg,msg_length,0);
        int received_bytes = recv(client_fd,received_msg,MAX_DATA,0);
        cout<<"Message received by the server and echoed back to Client 2 is: "<<received_msg<<endl;
    }
