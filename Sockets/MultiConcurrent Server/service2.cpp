...
int main()
{
    ...
    cout<<"Service 2 has started\n";
    int received_bytes = recv(nsfd,buffer,MAX_DATA,0);
    buffer[received_bytes]=0;
    cout<<"Message received by Service 2: "<<buffer<<endl;
    cout<<"Service 2 : Sending message to the client\n";
    fgets(buf_sent,MAX_DATA,stdin);
    send(nsfd,buf_sent,strlen(buf_sent)+1,0);
    buf_sent[strlen(buf_sent)]=0;
    cout<<"Service 2: Message send "<<buf_sent<<endl;
    close(nsfd);
    return 0;
}